...
public partial class ClientViewModel : ObservableObject
{
    ...
    // Handikap — tek taraf seçim sistemi
    // 0 = yok, 1 = Player A, 2 = Player B
    [ObservableProperty]
    private int _handikapOyuncu = 0;

    [ObservableProperty]
    private int _handikapDeger = 0;

    public bool IsHandikapNone => HandikapOyuncu == 0;
    public bool IsHandikapA => HandikapOyuncu == 1;
    public bool IsHandikapB => HandikapOyuncu == 2;

    partial void OnHandikapOyuncuChanged(int value)
    {
        OnPropertyChanged(nameof(IsHandikapNone));
        OnPropertyChanged(nameof(IsHandikapA));
        OnPropertyChanged(nameof(IsHandikapB));
    }

    [RelayCommand]
    private void SelectHandikapA() => HandikapOyuncu = HandikapOyuncu == 1 ? 0 : 1;

    [RelayCommand]
    private void SelectHandikapB() => HandikapOyuncu = HandikapOyuncu == 2 ? 0 : 2;

    [RelayCommand]
    private void HandikapUp() { if (HandikapOyuncu != 0) HandikapDeger++; }

    [RelayCommand]
    private void HandikapDown() { if (HandikapDeger > 0) HandikapDeger--; }
    ...
    public bool IsPlayerATurn => ActivePlayer == 1;
    public bool IsPlayerBTurn => ActivePlayer == 2;

    partial void OnActivePlayerChanged(int value)
    {
        OnPropertyChanged(nameof(IsPlayerATurn));
        OnPropertyChanged(nameof(IsPlayerBTurn));
    }
